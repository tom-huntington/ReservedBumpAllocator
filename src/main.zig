...
fn bytesToArray(allocator: *ReservedBufferAllocator, bytes: []const u8) !types.Array {
    ...
    const meta = types.MetadataHeader.initWithShape(allocator, .Exclusive, &.{bytes.len});
...
pub fn main() !void {
    ...
    const arg_meta = types.MetadataHeader.initWithShape(&runtime_alloc, .Exclusive, &.{8});
