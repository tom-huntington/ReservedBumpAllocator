...
pub const Parser = struct {
    ...
    fn materializeLiteralArray(self: *Parser, items: []const Value) ParseError!Value {
        ...
        const meta = types.Metadata.initWithShape(self.value_allocator, .Exclusive, shape);
    ...
    fn parseRawStringValue(self: *Parser, lexeme: []const u8) ParseError!Value {
        ...
        const meta = types.Metadata.initWithShape(self.value_allocator, .Exclusive, &.{bytes.len});
        for (bytes, 0..) |byte, i| {
