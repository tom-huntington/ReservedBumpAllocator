...
pub const Parser = struct {
    ...
    fn parseBracketLiteralValue(self: *Parser, index: *usize, end_index: usize) ParseError!Value {
        ...
        if (rows.items.len == 0) {
            ...
            const meta = types.MetadataHeader.initWithShape(self.value_allocator, .Exclusive, &.{ 0, 0 });
    ...
    fn materializeLiteralArray(self: *Parser, items: []const Value) ParseError!Value {
        ...
        const meta = types.MetadataHeader.initWithShape(self.value_allocator, .Exclusive, shape);
    ...
    fn parseRawStringValue(self: *Parser, lexeme: []const u8) ParseError!Value {
        ...
        const meta = types.MetadataHeader.initWithShape(self.value_allocator, .Exclusive, &.{bytes.len});
        for (bytes, 0..) |byte, i| {
